...
app.post('/albums', (req, res) => {
  ...
  const sql = 'INSERT INTO albums (user_id, title, description) VALUES (?, ?, ?)';
  db.query(sql, [user_id, title, description], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao criar álbum.' });
    res.status(201).json({ message: 'Álbum criado com sucesso!', albumId: result.insertId });
  });
});
...
app.get('/albums/:user_id', (req, res) => {
  ...
  db.query(sql, [user_id], (err, results) => {
    if (err) return res.status(500).json({ message: 'Erro ao buscar álbuns.' });
    res.json(results);
  });
});
...
app.post('/photos', (req, res) => {
  ...
  db.query(sql, [album_id, filename, url], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao adicionar foto.' });
    res.status(201).json({ message: 'Foto adicionada com sucesso!', photoId: result.insertId });
  });
});
...
app.put('/albums/:id', (req, res) => {
  ...
  db.query(sql, [title, description, id], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao editar álbum.' });
...
app.delete('/albums/:id', (req, res) => {
  ...
  db.query(sql, [id], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao deletar álbum.' });
...
app.put('/photos/:id', (req, res) => {
  ...
  db.query(sql, [filename, url, id], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao editar foto.' });
...
app.delete('/photos/:id', (req, res) => {
  ...
  db.query(sql, [id], (err, result) => {
    if (err) return res.status(500).json({ message: 'Erro ao deletar foto.' });
